...
class Blip2Qformer(Blip2Base):
    ...
    def forward(self, samples, is_train=True):
        ...
        epoch = samples.get("epoch", 0)  # <- runner already injects this

        # 2. Flatten to tokens
        rgb_tokens_raw   = self.rgb_layernorm(image.flatten(2).transpose(1, 2))   # [B, N, D]
        lidar_tokens_raw = self.lidar_layernorm(lidar.flatten(2).transpose(1, 2)) # [B, N, D]
        ...
        # === build random BLOCK masks (per batch) ===
        cam_mask = None      # [B, N] boolean
        lidar_mask = None    # [B, N] boolean
        is_train = False
        # if is_train:
        #     # 1) schedule block size over epochs
        #     start_block = 4
        #     max_block   = 16
        #     grow_every  = 5  # epochs

        #     block_size = min(start_block + (epoch // grow_every) * 2, max_block)

        #     # 2) schedule mask ratio over epochs
        #     start_ratio = 0.05   # 5% at beginning
        #     max_ratio   = 0.30   # up to 30%
        #     ratio_warmup_epochs = 20.0

        #     alpha = min(epoch / ratio_warmup_epochs, 1.0)   # in [0, 1]
        #     mask_ratio = start_ratio + alpha * (max_ratio - start_ratio)

        #     # 3) make BLOCK masks in BEV space [B, H, W]
        #     cam_mask_hw = self.make_block_mask(
        #         bs, H, W,
        #         block_size,
        #         mask_ratio,
        #         rgb_tokens.device,
        ...
        #     lidar_mask_hw = self.make_block_mask(
        #         bs, H, W,
        #         block_size,
        #         mask_ratio,
        #         lidar_tokens.device,
        ...
        #     # 4) flatten to [B, N] to align with tokens
        #     cam_mask   = cam_mask_hw.view(bs, -1)    # [B, N]
        #     lidar_mask = lidar_mask_hw.view(bs, -1)  # [B, N]

        #     # optional debug once per epoch / first iter
        #     if samples.get("iters", 0) == 0:
        #         cam_cells_ratio = cam_mask.float().mean().item()
        #         lid_cells_ratio = lidar_mask.float().mean().item()
        #         cam_blocks_est = (cam_cells_ratio * H * W) / (block_size ** 2)
        #         lid_blocks_est = (lid_cells_ratio * H * W) / (block_size ** 2)
        #         print(
        #             f"[DEBUG] epoch={epoch}, mask_ratio_sched={mask_ratio:.3f}, "
        #             f"block_size={block_size}, "
        #             f"cam_cells_ratio={cam_cells_ratio:.3f}, lid_cells_ratio={lid_cells_ratio:.3f}, "
        #             f"cam_blocks_est={cam_blocks_est:.1f}, lid_blocks_est={lid_blocks_est:.1f}"
        #         )



        #     # DEBUG: print fraction of masked tokens occasionally
        #     if samples.get("iters", 0) == 0 and samples.get("epoch", 0) == 0:
        #         cam_frac = cam_mask.float().mean().item()
        #         lid_frac = lidar_mask.float().mean().item()
        #         print(f"[DEBUG] mask_ratio={self.mask_ratio:.3f}, "
        #             f"cam_mask_frac={cam_frac:.3f}, lid_mask_frac={lid_frac:.3f}")


        # else:
        # === full masking during evaluation ===
        cam_mask = torch.ones(bs, H * W, dtype=torch.bool, device=rgb_tokens.device)
        lidar_mask = torch.ones(bs, H * W, dtype=torch.bool, device=lidar_tokens.device)

        # # DEBUG: how many cells & approx how many blocks
        # if samples.get("iters", 0) == 0 and samples.get("epoch", 0) == 0:
        #     cam_cells_masked = cam_mask.sum(dim=1)        # [B]
        #     lid_cells_masked = lidar_mask.sum(dim=1)      # [B]

        #     # approximate #blocks = (#masked cells) / (block_size^2)
        #     cam_blocks_est = (cam_cells_masked.float() / (block_size ** 2)).mean().item()
        #     lid_blocks_est = (lid_cells_masked.float() / (block_size ** 2)).mean().item()

        #     cam_ratio = (cam_cells_masked.float() / (H * W)).mean().item()
        #     lid_ratio = (lid_cells_masked.float() / (H * W)).mean().item()

        #     print(
        #         f"[DEBUG] mask_ratio={self.mask_ratio:.3f}, "
        #         f"block_size={block_size}, "
        #         f"cam_cells_ratio={cam_ratio:.3f}, lid_cells_ratio={lid_ratio:.3f}, "
        #         f"cam_blocks_est={cam_blocks_est:.1f}, lid_blocks_est={lid_blocks_est:.1f}"
        #     )

        # 4. Run 4 fusion layers
        cam_out, lid_out = rgb_tokens, lidar_tokens
        def fusion_stack(cam_in, lid_in, cam_mask, lidar_mask):
            for layer in self.fusion_layers:
                cam_in, lid_in = layer(cam_in, lid_in, cam_mask=cam_mask, lidar_mask=lidar_mask)
            return cam_in, lid_in

        if self.use_grad_checkpoint and is_train:
            cam_out, lid_out = checkpoint(
                fusion_stack, cam_out, lid_out, cam_mask, lidar_mask, use_reentrant=False
            )
        else:
            cam_out, lid_out = fusion_stack(cam_out, lid_out, cam_mask, lidar_mask)
