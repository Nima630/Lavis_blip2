...
class Blip2Qformer(Blip2Base):
    ...
    #     # === Prepare RGB Embeddings ===
    #     rgb_embeds = rgb_proj.flatten(2).transpose(1, 2)
    #     rgb_embeds = self.rgb_layernorm(rgb_embeds)
        
    #     # === Added positional encoding for RGB ===
    #     H_rgb, W_rgb = rgb_proj.shape[-2], rgb_proj.shape[-1]
    #     if not hasattr(self, '_cached_rgb_hw') or self._cached_rgb_hw != (H_rgb, W_rgb):
    #         self._cached_rgb_pos = self._build_2d_sincos_pos_embed(
    #             H_rgb, W_rgb, rgb_embeds.size(-1), rgb_embeds.device
    #         )
    ...
    #     # === Prepare LiDAR Embeddings ===
    #     lidar_embeds = lidar_proj.flatten(2).transpose(1, 2)
    #     lidar_embeds = self.lidar_layernorm(lidar_embeds)
        
    #     # === Added positional encoding for LiDAR ===
    #     H_lidar, W_lidar = lidar_proj.shape[-2], lidar_proj.shape[-1]
    #     if not hasattr(self, '_cached_lidar_hw') or self._cached_lidar_hw != (H_lidar, W_lidar):
    #         self._cached_lidar_pos = self._build_2d_sincos_pos_embed(
    #             H_lidar, W_lidar, lidar_embeds.size(-1), lidar_embeds.device
    #         )
    ...
    # # for testing / feature extraction with no shuffling
