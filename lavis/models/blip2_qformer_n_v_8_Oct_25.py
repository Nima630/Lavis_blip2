...
class ChannelFiLMMasked(nn.Module):
    ...
    def masked_gap(self, x, mask=None):
        # x: [B, C, H, W]; mask: [B, 1, H, W]
        if mask is None:
            return x.mean(dim=(2,3))  # [B, C]
        masked = x * mask
        denom = mask.sum(dim=(2,3)).clamp(min=1e-6)  # [B, 1]
        return masked.sum(dim=(2,3)) / denom         # [B, C]

    def forward(self, target_feat, cond_feat, cond_mask=None, remask_after=None):
        """
        target_feat:  [B, C_out, Ht, Wt]   -> the feature we modulate
        cond_feat:    [B, C_in,  Hc, Wc]   -> the feature we pool to produce γ, β
        cond_mask:    [B, 1,     Hc, Wc]   -> optional mask for cond_feat
        remask_after: [B, 1,     Ht, Wt]   -> optional mask to re-apply on the output
        """
        B, C_out, _, _ = target_feat.shape
        cond_vec = self.masked_gap(cond_feat, cond_mask)       # [B, C_in]

        gamma = self.gamma_mlp(cond_vec).view(B, C_out, 1, 1)  # [B, C_out, 1, 1]
        beta  = self.beta_mlp(cond_vec).view(B, C_out, 1, 1)   # [B, C_out, 1, 1]

        out = gamma * target_feat + beta
        if remask_after is not None:
            out = out * remask_after



        # print("[DEBUG] gamma stats:")
        # print(" - shape:", gamma.shape)
        # print(" - mean:", gamma.mean().item())
        # print(" - std:", gamma.std().item())
        # print(" - min/max:", gamma.min().item(), gamma.max().item())

        # print("[DEBUG] beta stats:")
        # print(" - shape:", beta.shape)
        # print(" - mean:", beta.mean().item())
        # print(" - std:", beta.std().item())
        # print(" - min/max:", beta.min().item(), beta.max().item())
    
        return out


class DualChannelFiLM(nn.Module):
    def __init__(self, c_rgb, c_lidar, reduction=16):
        super().__init__()
        self.lidar2rgb = ChannelFiLMMasked(c_lidar, c_rgb, reduction)
        self.rgb2lidar = ChannelFiLMMasked(c_rgb, c_lidar, reduction)

    def forward(self, rgb_feat, lidar_feat, lidar_mask=None):
        """
        rgb_feat:   [B, C_rgb, Hr, Wr]
        lidar_feat: [B, C_lid, Hl, Wl]
        lidar_mask: [B, 1,     Hl, Wl]  (front-FOV mask in LiDAR plane)
        """
        # 1) LiDAR -> RGB (masked pooling on LiDAR; RGB has no spatial mask)
        rgb_mod = self.lidar2rgb(
            target_feat=rgb_feat,
            cond_feat=lidar_feat,
            cond_mask=lidar_mask,      # so zeros don't pollute the descriptor
            remask_after=None          # RGB fully valid
        )

        # 2) RGB -> LiDAR (no mask on RGB pooling, but re-mask LiDAR after FiLM)
        lidar_mod = self.rgb2lidar(
            target_feat=lidar_feat,
            cond_feat=rgb_feat,
            cond_mask=None,            # RGB is fully valid
            remask_after=lidar_mask    # keep LiDAR zeros zero
        )

        return rgb_mod, lidar_mod
...
class Blip2Qformer(Blip2Base):
    ...
    def __init__(
        ...
        # # === NEW: learnable positional embeddings for the query tokens ===
        # self.query_pos_rgb   = nn.Parameter(torch.zeros(1, num_query_token, hidden_dim))
        # self.query_pos_lidar = nn.Parameter(torch.zeros(1, num_query_token, hidden_dim))
        # nn.init.trunc_normal_(self.query_pos_rgb, std=0.02)
        # nn.init.trunc_normal_(self.query_pos_lidar, std=0.02)
        ...
        # for FiLM
        # self.dual_film = DualChannelFiLM(c_rgb=hidden_dim, c_lidar=hidden_dim, reduction=16)

        # # === Load static LiDAR mask ===
        # mask_path = "./result/lidar_mask_train_fov.png"
        # pil_mask = Image.open(mask_path).convert("L")  # grayscale
        # resize = T.Resize((90, 90), interpolation=Image.NEAREST) # # Resize to match LiDAR projected feature map size
        # mask_resized = resize(pil_mask)
                
        # # Convert to tensor and binarize
        # mask_tensor = T.ToTensor()(mask_resized) > 0.5  # [1, 90, 90], bool
        # self.register_buffer("lidar_mask_static", mask_tensor.float())  # [1, 90, 90]
        # self.lidar_mask_static = torch.load("./result/lidar_mask_static.pt").to(device)
        
        # mask_path = "./result/lidar_mask_tensor.pt"
        # device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
        # self.lidar_mask_static = torch.load(mask_path).to(device) 
    ...
    # 
    # def forward_position(self, samples, is_train=True):
    ...
    #     # === Positional embeddings for encoder_hidden_states (context) ===
    #     H_rgb, W_rgb = rgb_proj.shape[-2], rgb_proj.shape[-1]
    #     if self._cached_rgb_hw != (H_rgb, W_rgb):
    #         self._cached_rgb_pos = self._build_2d_sincos_pos_embed(H_rgb, W_rgb, rgb_embeds.size(-1), rgb_embeds.device)
    #         self._cached_rgb_hw = (H_rgb, W_rgb)
    #     pos_rgb = self._cached_rgb_pos  # [1, H*W, D]
    #     rgb_embeds = rgb_embeds + pos_rgb
    #     rgb_atts = torch.ones(rgb_embeds.size()[:-1], dtype=torch.long).to(image.device)
    #     rgb_atts = torch.ones(rgb_embeds.size()[:-1], dtype=torch.long).to(image.device)
    ...
    #     H_lidar, W_lidar = lidar_proj.shape[-2], lidar_proj.shape[-1]
    #     if self._cached_lidar_hw != (H_lidar, W_lidar):
    #         self._cached_lidar_pos = self._build_2d_sincos_pos_embed(H_lidar, W_lidar, lidar_embeds.size(-1), lidar_embeds.device)
    #         self._cached_lidar_hw = (H_lidar, W_lidar)
    #     pos_lidar = self._cached_lidar_pos  # [1, H*W, D]
    #     lidar_embeds = lidar_embeds + pos_lidar

    #     lidar_atts = torch.ones(lidar_embeds.size()[:-1], dtype=torch.long).to(lidar.device)
        
    #     # === Positional embeddings for query tokens ===
    #     query_tokens       = (self.query_tokens       + self.query_pos_rgb  ).expand(bs, -1, -1)
    #     query_tokens_lidar = (self.query_tokens_lidar + self.query_pos_lidar).expand(bs, -1, -1)
    ...
    #     # global_step = samples.get("global_step", None)
    ...
    # # printing forward mini FiLM (masked)
    # def forward(self, samples, is_train=True):
    #     # print("\n====== FORWARD PASS START ======")
        
    #     shuffle_prob = 0.5
    #     image = samples["image"]           # [B, 3, H, W]
    #     lidar = samples["lidar"]           # [B, 1, C, H_l, W_l]
    #     bs = image.size(0)
        
    #     # print(f"[INPUT] image: {image.shape}")
    #     # print(f"[INPUT] lidar: {lidar.shape}")
        
    #     # nromal projection 
    #     rgb_proj = self.rgb_input_proj(image)  # [B, C, H_r, W_r]
    #     # print(f"[STEP] rgb_proj: {rgb_proj.shape}")

    #     lidar_feat = lidar.squeeze(1)  # [B, C_l, H_l, W_l]
    #     # print(f"[STEP] lidar_feat (squeezed): {lidar_feat.shape}")

    #     lidar_proj = self.lidar_input_proj(lidar_feat)  # [B, C, H_l, W_l]
    #     # print(f"[STEP] lidar_proj: {lidar_proj.shape}")
        
    #     # adding FiLM
    #     # print("+++++++++++++++++++++++++++++++++++++++++++++++++++")

    #     # lidar_mask = self.lidar_mask_static.expand(bs, 1, 90, 90)
    #     lidar_mask = self.lidar_mask_static.to(lidar.device).expand(bs, 1, 90, 90)

    #     # lidar_mask: [B, 1, H, W] with 0s (masked) and 1s (unmasked)

    #     num_masked = (lidar_mask == 0).sum().item()
    #     num_unmasked = (lidar_mask == 1).sum().item()
    #     total = lidar_mask.numel()

    #     # print("[DEBUG] lidar_mask stats:")
    #     # print(" - shape:", lidar_mask.shape)
    #     # print(" - min:", lidar_mask.min().item())
    #     # print(" - max:", lidar_mask.max().item())
    #     # print(" - unique values:", torch.unique(lidar_mask))
    #     # print(f" - masked cells (0):     {num_masked:,}")
    #     # print(f" - unmasked cells (1):   {num_unmasked:,}")
    #     # print(f" - total cells:          {total:,}")


    #     rgb_proj, lidar_proj = self.dual_film(rgb_proj, lidar_proj, lidar_mask=lidar_mask)


    #     # ================= RGB Branch ====================
    #     rgb_embeds = rgb_proj.flatten(2).transpose(1, 2)  # [B, H_r*W_r, C]
    #     # print(f"[STEP] rgb_embeds (flattened): {rgb_embeds.shape}")

    #     rgb_embeds = self.rgb_layernorm(rgb_embeds)
    #     # print(f"[STEP] rgb_embeds (layernorm): {rgb_embeds.shape}")

    #     rgb_atts = torch.ones(rgb_embeds.size()[:-1], dtype=torch.long).to(image.device)
    #     # print(f"[STEP] rgb_atts: {rgb_atts.shape}")

    #     query_tokens = self.query_tokens.expand(bs, -1, -1)
    #     # print(f"[STEP] query_tokens: {query_tokens.shape}")
    ...
    #         return_dict=True,
    #     )
    #     # print(f"[STEP] rgb_output.last_hidden_state: {rgb_output.last_hidden_state.shape}")

    #     rgb_feats = F.normalize(self.vision_proj(rgb_output.last_hidden_state), dim=-1)
    #     # print(f"[STEP] rgb_feats (projected + normalized): {rgb_feats.shape}")  # [B, N_q, D]

    #     # ================= LiDAR Branch ====================


    #     lidar_embeds = lidar_proj.flatten(2).transpose(1, 2)  # [B, H_l*W_l, C]
    #     # print(f"[STEP] lidar_embeds (flattened): {lidar_embeds.shape}")

    #     lidar_embeds = self.lidar_layernorm(lidar_embeds)
    #     # print(f"[STEP] lidar_embeds (layernorm): {lidar_embeds.shape}")

    #     lidar_atts = torch.ones(lidar_embeds.size()[:-1], dtype=torch.long).to(lidar.device)
    #     # print(f"[STEP] lidar_atts: {lidar_atts.shape}")

    #     query_tokens_lidar = self.query_tokens_lidar.expand(bs, -1, -1)
    #     # print(f"[STEP] query_tokens_lidar: {query_tokens_lidar.shape}")
    ...
    #         return_dict=True,
    #     )
    #     # print(f"[STEP] lidar_output.last_hidden_state: {lidar_output.last_hidden_state.shape}")

    #     lidar_feats = F.normalize(self.lidar_proj(lidar_output.last_hidden_state), dim=-1)
    #     # print(f"[STEP] lidar_feats (projected + normalized): {lidar_feats.shape}")

    #     # ================= Pool for similarity ====================
    #     rgb_avg = rgb_feats.mean(dim=1)     # [B, D]
    #     lidar_avg = lidar_feats.mean(dim=1) # [B, D]

    #     # print(f"[POOL] rgb_avg: {rgb_avg.shape}")
    #     # print(f"[POOL] lidar_avg: {lidar_avg.shape}")

    #     # ================= Optionally shuffle lidar feats ====================
    #     if is_train and shuffle_prob > 0: 
    #         device = image.device
    #         perm = torch.randperm(bs, device=device)
    #         keep_mask = torch.rand(bs, device=device) > shuffle_prob
    ...
    #         targets = torch.arange(bs, device=device)
    #         targets[~keep_mask] = perm[~keep_mask]
    #         # print(f"[SHUFFLE] Number of mismatches introduced: {(~keep_mask).sum().item()}")
    #     else:
    #         lidar_avg_shuffled = lidar_avg
    #         targets = torch.arange(bs, device=image.device)
    ...
    #     # ================= Contrastive Loss ====================
    #     sim_matrix = (rgb_avg @ lidar_avg_shuffled.T) * torch.exp(self.log_temp)
    #     # print(f"[SIM] sim_matrix: {sim_matrix.shape}")
    #     # print(f"[SIM] targets: {targets.shape} (values: {targets.tolist()})")

    #     loss_contrastive = (
    #         F.cross_entropy(sim_matrix, targets) + 
    #         F.cross_entropy(sim_matrix.T, targets)) / 2

    #     # print(f"[LOSS] Contrastive loss: {loss_contrastive.item():.4f}")
    ...
    # def forward_pos(self, samples, is_train=True):
    #     print("forward   ")
    ...
    def forward_features_shuffled(self, samples, shuffle_prob=0.5, is_train=False):
        """
        ...
        lidar_embeds = self.lidar_layernorm(lidar_embeds)
         ...
         # === Attack injection (only during test) ===
        attack_flags = torch.zeros(bs, dtype=torch.bool, device=image.device)
        if attack_prob>0:
            attack_flags = torch.rand(bs, device=image.device) < attack_prob

            def add_noise(x, std=0.3): return x + torch.randn_like(x) * std
            def shift_tokens(x, shift=5): return torch.roll(x, shifts=shift, dims=1)

            rgb_embeds[attack_flags] = add_noise(rgb_embeds[attack_flags])
            lidar_embeds[attack_flags] = shift_tokens(lidar_embeds[attack_flags])
        ...
        rgb_avg = rgb_feats.mean(dim=1)      # [B, D]
        lidar_avg = lidar_feats.mean(dim=1)  # [B, D]
        cos_sim_diag = F.cosine_similarity(rgb_avg, lidar_avg, dim=-1)  # [B] # +++++++++++++++++++++++++++++++++++++++++++++++++++++
        ...
        return {
            ...
            "cos_sim_diag": cos_sim_diag,  # [B], similarity of matched pairs  # +++++++++++++++++++++++++++++++++++++++++++++++++++++
