...
class TorchLidarDataset:
    ...
    def __getitem__(self, idx):
        ...
        try:
            lidar_data = torch.load(path, weights_only=False) 
            ...
            img_shape_raw = tuple(camera_data['img_shape'])[:2]  # (H, W)

            if isinstance(img_shape_raw, (tuple, list)) and isinstance(img_shape_raw[0], (int, float)):
                img_shape = tuple(img_shape_raw)[:2]  # flat format
            else:
                img_shape = tuple(img_shape_raw[0])[:2]  # nested format
